...
-(void)_testJasmine {
    ...
    TransitFunction *onFinish = [[TransitNativeFunction alloc] initWithContext:context nativeId:@"onFinish" genericBlock:^id(TransitNativeFunctionCallScope *scope) {
        ...
        finished = YES;
        XCTAssertEqualObjects(@0, results[@"failed"], @"no test failed");
        XCTAssertTrue([results[@"passed"] intValue] >= 51, @"at the time of writing, 51 tests should have passed");
        return @"finished :)";
    }];
    ...
    XCTAssertEqualObjects(@"Jasmine Spec Runner", [context eval:@"document.title"], @"page loaded");
...
-(void)testThrowsExceptionIfDelegateReplaced {
    ...
    XCTAssertTrue(context.webView.delegate == context, @"delegate wired");

    id otherDelegate = [NSObject new];
    XCTAssertThrows([context.webView setDelegate: otherDelegate], @"delegate property must not be replaced");
    XCTAssertTrue(context.webView.delegate == otherDelegate, @"delegate replaced");
}
...
-(void)testOriginalDelegateWillBeCalled {
    ...
    XCTAssertTrue(webView.delegate == mockedDelegate, @"delegate wired to mock");

    TransitUIWebViewContext *context = [TransitUIWebViewContext contextWithUIWebView:webView];

    XCTAssertTrue(webView.delegate == context, @"delegate wired to context");
    ...
    XCTAssertNoThrow([mockedDelegate verify], @"verify");
}
